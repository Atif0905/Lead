...
const AssignPopup4 = ({ leadId, setIsAssignLead, deals, setDeals }) => {
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (usersResponse.data.status === "ok") {
            const teamlead = usersResponse.data.data.filter(user => user.userType === 'Executive');
            setUsers(teamlead);
            console.log('Directors:', teamlead);
          } else {
